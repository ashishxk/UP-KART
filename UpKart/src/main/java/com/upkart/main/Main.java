...
import com.upkart.model.Category;
import com.upkart.model.Devices;
import com.upkart.model.ElectronicsBrand;
import com.upkart.model.PaymentOption;
		...
		do {
			...
			if (choice == 1) {
				...
				case 4:
					...
					List<Admin> adminCart = null;

					try {
						adminCart = adminRef.getProductsInfo();
					} catch (ProductNotAvailableException e) {
						System.out.println(e.getMessage());
					}

					if (adminCart != null) {
						for (Admin products : adminCart) {
				...
				if (userChoice == 1) {
					...
					if (check != null) {
						...
						case 1:
							...
							String category = Category.values()[productCategory].category;
							...
							if (productCategory == 1) {
								...
								String brand = ElectronicsBrand.values()[productBrand].brand;

								System.out.println("SELECT DEVICE");
								System.out.println("1. LAPTOP");
								System.out.println("2. MOBILE");

								int deviceName = sc.nextInt();

								String productName = Devices.values()[deviceName].productName;
								...
								int deliveryPaymentOption = sc.nextInt();

								String paymentOption = PaymentOption.values()[deliveryPaymentOption].paymentOption;

								System.out.println("ENTER QUANTITY");
								int quantity = sc.nextInt();

								User addToCart = new User(username, category, brand, productName, paymentOption,
										quantity);
						...
						case 2:
							...
							List<User> userCart = null;

							try {
								userCart = userRef.viewCart(userNameLogin);
							} catch (EmptyCartException e) {
								System.out.println(e.getMessage());
							}

							if (userCart != null) {
								for (User items : userCart) {
						...
						case 3:
							...
							userRef.emptyCart(categoryTODelete, productToDelete);
					...
					userRef.deleteAccount(userNameToDelete);
